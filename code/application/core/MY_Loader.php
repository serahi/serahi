...
class MY_Loader extends MX_Loader
{
	public function _ci_load($_ci_data)
	{
		...
		foreach (array('_ci_view', '_ci_vars', '_ci_path',
			...
			$$_ci_val = (!isset($_ci_data[$_ci_val])) ? FALSE : 
			            $_ci_data[$_ci_val];
		}
		...
		$_ci_dir = substr($_ci_path, 0, strlen($_ci_path) - 
		                  strlen($_ci_file));
		$_ci_relative = substr($_ci_path, strlen(APPPATH));
		$CI->load->library('smarty', APPPATH);
		
		if (is_array($_ci_vars)) {
			$this->_ci_cached_vars = array_merge($this->_ci_cached_vars,
			                                     $_ci_vars);
		...
		$str = $CI->smarty->fetch('extends:templates/template.php|' . 
		                          $_ci_relative);
